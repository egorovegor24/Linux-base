...
-  Проверить файл hosts.
   ...
   Для этого, необходимо пройти в «Мой компьютер», или «Домашняя папка», или «Менеджер файлов» в поиске ввести: «etc». Необходимо найти файл под названием «hosts» и открыть его. Если не нашли папку «etc», то можно проверить содержимое hosts через терминал под суперпользователем командой cat /etc/hosts или sudo nano /etc/hostscat /etc/hosts или sudo nano /etc/hosts
